...
async def bot_start(message: types.Message, state: FSMContext, bot: Bot, command: CommandObject):
    args = command.args
    print(args)
    if args == 'telegram_id':
    ...
    if not user:
        await message.answer('Приветствую тебя! Кажется мы еще не знакомы\n'
                             'Зарегистрируйся на sop-dashboard.ru\n'
                             f'Тебе понадобится твой ID: {message.from_user.id}')
